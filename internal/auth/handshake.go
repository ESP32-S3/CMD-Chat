...
type Response struct {
	...
	// Name is the sender's chosen nickname. It is deliberately outside the
	// signed payload: it is a display label, not a claim, and signing it would
	// suggest it carries the same weight as the ID. Only the ID is proven.
	//
	// The field is optional, so a peer running an older build simply omits it
	// and is shown by ID instead.
...
func Respond(id *identity.Identity, challenge *Challenge) (*Response, error) {
	return RespondAs(id, challenge, "")
}

// RespondAs is Respond with a display nickname attached.
//
// The nickname is not signed and is not part of authentication; see Response.Name.
func RespondAs(id *identity.Identity, challenge *Challenge, name string) (*Response, error) {
	if challenge == nil || challenge.Nonce == "" {
		return nil, errors.New("missing authentication challenge")
	}
...
func Verify(challenge *Challenge, response *Response) error {
	if challenge == nil || response == nil || response.Nonce != challenge.Nonce {
		return errors.New("authentication nonce mismatch")
	}
	pub, err := base64.StdEncoding.DecodeString(response.PublicKey)
	if err != nil {
		return err
	}
	...
	if response.ID != expected {
		return errors.New("identity does not match public key")
	}
...
func base32NoPadding(b []byte) string {
	...
	if bits > 0 {
		out = append(out, alphabet[(acc<<(5-bits))&31])
	}
	return string(out)
}
