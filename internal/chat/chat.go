...
func (h *Host) handle(c net.Conn) {
	defer c.Close(); dec:=json.NewDecoder(bufio.NewReader(c))
	// Mutual cryptographic authentication: client challenges host, then host challenges client.
	var challenge auth.Challenge
	if err:=dec.Decode(&challenge); err!=nil || challenge.Type!="auth_challenge" { return }
	hostResponse, err:=auth.Respond(h.Identity,&challenge); if err!=nil{return}
	if err=h.writeJSON(c,hostResponse); err!=nil{return}
	ownChallenge, err:=auth.NewChallenge(); if err!=nil{return}; if err=h.writeJSON(c,ownChallenge); err!=nil{return}
	var clientResponse auth.Response; if err=dec.Decode(&clientResponse); err!=nil{return}; if err=auth.Verify(ownChallenge,&clientResponse); err!=nil { _=h.writePacket(c,Packet{Type:"error",Text:"authentication failed"}); return }

	h.Mu.Lock(); h.Clients[c]=struct{}{}; h.Mu.Unlock(); defer func(){h.Mu.Lock();delete(h.Clients,c);h.Mu.Unlock()}()
	if err=h.writePacket(c,Packet{Type:"hello",From:h.ID,Name:h.Name}); err!=nil{return}
	for { var p Packet; if err:=dec.Decode(&p); err!=nil{return}; if p.Type!="msg"{continue}; if len([]byte(p.Text))>MaxMessageBytes{_ = h.writePacket(c,Packet{Type:"error",Text:"message exceeds 4096 bytes"});continue}; fmt.Printf("\r[%s] %s\n> ",p.Name,p.Text); h.broadcast(Packet{Type:"msg",From:p.From,Name:p.Name,Text:p.Text},c) }
}
...
func Client(address, expectedFingerprint, expectedHostID, id, name string, ident *identity.Identity) (net.Conn,*json.Decoder,error) {
	c,err:=tls.Dial("tcp",address,&tls.Config{MinVersion:tls.VersionTLS13,InsecureSkipVerify:true});if err!=nil{return nil,nil,err}
	state:=c.ConnectionState();if len(state.PeerCertificates)==0{_ = c.Close();return nil,nil,errors.New("host sent no certificate")};sum:=sha256.Sum256(state.PeerCertificates[0].Raw);actual:=hex.EncodeToString(sum[:]);expected:=strings.TrimSpace(expectedFingerprint);if expected==""{fmt.Println("Warning: host certificate is not pinned; identity authentication is still required.")}else if !strings.EqualFold(actual,expected){_ = c.Close();return nil,nil,fmt.Errorf("host fingerprint mismatch")}
	reader:=bufio.NewReader(c);dec:=json.NewDecoder(reader)
	challenge,err:=auth.NewChallenge();if err!=nil{_ = c.Close();return nil,nil,err};if err=json.NewEncoder(c).Encode(challenge);err!=nil{_ = c.Close();return nil,nil,err}
	var hostResponse auth.Response;if err=dec.Decode(&hostResponse);err!=nil{_ = c.Close();return nil,nil,err};if err=auth.Verify(challenge,&hostResponse);err!=nil{_ = c.Close();return nil,nil,fmt.Errorf("host identity authentication failed: %w",err)};if expectedHostID!=""&&hostResponse.ID!=expectedHostID{_ = c.Close();return nil,nil,fmt.Errorf("host identity mismatch: expected %s, got %s",expectedHostID,hostResponse.ID)}
	var hostChallenge auth.Challenge;if err=dec.Decode(&hostChallenge);err!=nil{_ = c.Close();return nil,nil,err};response,err:=auth.Respond(ident,&hostChallenge);if err!=nil{_ = c.Close();return nil,nil,err};if err=json.NewEncoder(c).Encode(response);err!=nil{_ = c.Close();return nil,nil,err}
	return c,dec,nil
}
